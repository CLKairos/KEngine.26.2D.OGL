...
namespace system
{
  ...
  void Tile::update (entt::registry& registry, const double dt)
  {
    ...
    if (m_last_camera_x != camera_position.x || m_last_camera_y != camera_position.y)
    {
      m_tile_map.create_tile_entities (
          camera_position.x - m_tile_map.tile_dimension() * m_frustrum_culling_offset,
          camera_position.x + camera_dimension.w + m_tile_map.tile_dimension() + m_tile_map.tile_dimension() * m_frustrum_culling_offset,
          camera_position.y - m_tile_map.tile_dimension() * m_frustrum_culling_offset,
          camera_position.y + camera_dimension.h + m_tile_map.tile_dimension() + m_tile_map.tile_dimension() * m_frustrum_culling_offset);
      m_last_camera_x = camera_position.x;
      m_last_camera_y = camera_position.y;
    }
  ...
  void Tile::remove_tile_visibility (entt::registry& registry, entt::entity entity)
  {
    if (m_tile_map.has_loaded())
    {
      ...
      m_tile_map.layers[tile.layer_id]->set_visibility (pos.x / m_tile_map.tile_dimension(), pos.y / m_tile_map.tile_dimension(), false);
