#include "kengine/physics2d/grid_world.hpp"
#include "kengine/physics2d/body.hpp"
#include "kengine/physics2d/water_surface.hpp"
#include <algorithm>
#include <cmath>
#include <iostream>
...
namespace physics2d
{
  GridWorld::GridWorld (const core::TileMap& tile_map) : m_tile_map (tile_map) {}

  GridWorld::~GridWorld() { this->Bodies.clear(); }
  ...
  void GridWorld::AddBody (std::shared_ptr<kengine::physics2d::Body> body) { this->Bodies.emplace_back (body); }

  void GridWorld::RemoveBody (std::shared_ptr<kengine::physics2d::Body> body)
  {
  ...
  void GridWorld::add_water_surface (const std::shared_ptr<kengine::physics2d::WaterSurface>& water_surface)
  {
    m_water_surfaces.emplace_back (water_surface);
  }

  void GridWorld::remove_water_surface (const std::shared_ptr<kengine::physics2d::WaterSurface>& water_surface)
  {
  ...
  // bool GridWorld::intersects(const std::shared_ptr<Body> &body, const
  // core::Tile &tile)
  //{
  ...
  void GridWorld::resolve_aabb (const GridWorld::Collision& collision)
  {
    ...
    {
      // If tile has a active edge in the opposite direction of the body
      // (bottom)
  ...
  void GridWorld::UpdateMovement (float dt) const
  {
    for (auto& body : this->Bodies)
    {
      if (!body->IsStatic)
      {
        ...
        // Y movement
        if (body->Gravity && !body->Collisions.test (2))
          body->Acceleration.y += this->Gravity * dt;
        if (body->Gravity && body->Acceleration.y > this->Gravity)
