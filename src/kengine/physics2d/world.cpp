...
#include "kengine/physics2d/manifold.hpp"
...
namespace physics2d
{
  ...
  void World::AddBody (std::shared_ptr<kengine::physics2d::Body> body) { this->Bodies.emplace_back (body); }

  void World::RemoveBody (std::shared_ptr<kengine::physics2d::Body> body)
  {
  ...
  bool World::AABBvsAABB (std::shared_ptr<kengine::physics2d::Body> ba, std::shared_ptr<kengine::physics2d::Body> bb)
  {
    ...
    if (!ba->IsStatic || !bb->IsStatic)
    {
      ...
      if (!ba->IsStatic && bb->IsStatic)
      {
        kengine::physics2d::Manifold mf (ba, bb, glm::vec2 (overlapx, overlapy));
        this->ResolveManifold (mf);
      }
      else if (!bb->IsStatic && ba->IsStatic)
      {
        kengine::physics2d::Manifold mf (bb, ba, glm::vec2 (overlapx, overlapy));
        this->ResolveManifold (mf);
      }
      else
      {
  ...
  void World::ResolveManifold (kengine::physics2d::Manifold& mf)
  {
    // Shortcuts for:
    //    - Last *A* bottom
    //    - Last *A* top
