...
#include "kengine/graphics/shader.hpp"

#include "kengine/graphics/opengl.hpp" // IWYU pragma: export
...
namespace graphics
{
  Framebuffer::Framebuffer (kengine::graphics::Display& display) : Display (display) { this->init(); }
  ...
  void Framebuffer::RefreshTextureResolution()
  {
    ...
    glTexImage2D (GL_TEXTURE_2D,
                  0,
                  GL_RGBA,
                  this->CurrentTextureResolution.x,
                  this->CurrentTextureResolution.y,
                  0,
                  GL_RGBA,
                  GL_UNSIGNED_BYTE,
                  NULL);
